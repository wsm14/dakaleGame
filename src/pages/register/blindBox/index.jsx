...
import { backgroundObj, toast, cobyInfo, reloadTab } from '@/utils/utils';
  ...
  render() {
    ...
    return (
      <div className="blind_box">
        ...
        >
          <div className="blind_titleName">我的盲盒橱窗</div>
        </div>
        <div className="blind_key_info">
          {noUseKeys > 0 ? (
            <div className="blind_key_left">你有{noUseKeys}把金钥匙，快去拆盲盒大礼吧～</div>
          ) : hasKeys - noUseKeys === 4 ? (
            <div className="blind_key_left">所有盲盒都被你开启了，等待新盲盒</div>
          ) : (
        ...
        {prizeInfoList.map((item, index) => {
          ...
          const { prizeImg } = item;
          if (index === keys && noUseKeys > 0) {
            return (
              <div key={index} className={classNames(littSTyle)}>
                <Lottie
                  options={{
                    loop: true,
                    autoplay: true,
            ...
            return (
              <div
                key={index}
                style={backgroundObj(index < keys ? prizeImg : null)}
